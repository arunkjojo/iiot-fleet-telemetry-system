...
- **Docker**, to build the four custom images this chart deploys

### The four images are not on any public registry

`db`, `backend`, `frontend`, and `iiot-emitter` are all built from Dockerfiles in this repo — none
are published anywhere. **This is the single most likely first-run failure** (`ImagePullBackOff`
on every pod). Build each image, then either push it to a registry your cluster can pull from, or
load it directly into a local kind/minikube cluster:

```bash
docker build -t iiot-fleet-db ./db
docker build -t iiot-fleet-backend ./backend
docker build -t iiot-fleet-frontend ./frontend
docker build -t iiot-fleet-emitter ./iiot-emitter
```

For a local **kind** cluster:

```bash
kind load docker-image iiot-fleet-db iiot-fleet-backend iiot-fleet-frontend iiot-fleet-emitter
```
...
docker build -t iiot-fleet-db ./db   # repeat for backend, frontend, iiot-emitter
```
...
|-----|---------|--------------|
| `db.image.repository` | `iiot-fleet-db` | Image built from `db/Dockerfile` |
| `db.image.tag` | `latest` | Image tag |
...
| `backend.image.repository` | `iiot-fleet-backend` | Image built from `backend/Dockerfile` |
...
| `frontend.image.repository` | `iiot-fleet-frontend` | Image built from `frontend/Dockerfile` |
...
| `emitter.image.repository` | `iiot-fleet-emitter` | Image built from `iiot-emitter/Dockerfile` |
...
kind load docker-image iiot-fleet-db iiot-fleet-backend iiot-fleet-frontend iiot-fleet-emitter --name iiot-fleet
...
`docker-compose.yml` gates `backend`'s and `iiot-emitter`'s startup on the upstream service's
healthcheck passing (`depends_on: condition: service_healthy`). Kubernetes has no native
equivalent — Pods start independently regardless of other Pods' readiness. `emitter-deployment.yaml`
approximates this with an `initContainer` that polls the backend Service until it responds before
...
`docker-compose.yml` uses one named volume (`postgres_data`) shared implicitly by whichever
...
`docker-compose.yml`'s `backend` healthcheck uses a raw `/dev/tcp` bash check because the
`aspnet:8.0` base image has neither `curl` nor `wget`. The Helm chart's `backend-deployment.yaml`
...
The four images aren't on any public registry (see
[prerequisites](#the-four-images-are-not-on-any-public-registry)). Build them, then `kind load
